...
export async function GET(request: Request) {
    try {
        ...
        const targetForData = selectedTarget || sortedTargets[0] || null;
        const monthForData = selectedMonth || sortedMonths[0] || null;
        ...
        const monthlySummaryChart = {
            ...
            datasets: evaluationItemKeys.map((key, index) => ({
                ...
                data: lastSixMonths.map(month => {
                    ...
                    return key === 'potential' ? avg / 2 : avg; // Convert score for 'potential'
                }),
        ...
        const currentMonthValues = evaluationItemKeys.map(key => {
            ...
            return key === 'potential' ? avg / 2 : avg; // Convert score
        });
        ...
        return NextResponse.json({
            ...
            currentMonthAverage: numEvaluators > 0 ? ((Object.values(crossTabAverages).slice(1) as number[]).reduce((sum, val) => sum + val, 0) / (evaluationItemKeys.length)).toFixed(1) : "0.0",
            cumulativeAverage: targetEvalsAllMonths.length > 0 ? (cumulativeValues.reduce((sum, v) => sum + v, 0) / evaluationItemKeys.length).toFixed(1) : "0.0",
            filterOptions,
